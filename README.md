...
Implementation challenges: The challenging part was correctly implementing quicksort's partition logic - particularly getting the pivot swap and index boundaries right without errors and realizing that simply changing the pivot strategy (last vs middle element) has almost no effect on random data but a dramatic one on sorted data.
...
Binary search's real superiority only becomes undeniable at much larger n (tens of thousands or more), where O(log n) vs O(n) becomes the decisive factor. At n=1,000,000 binary search takes ~20 comparisons.

linear search takes ~500,000 on average.

Why does Binary Search require a sorted array?
Because it halves array every iteration and checks if the target is greater than or less than middle element
